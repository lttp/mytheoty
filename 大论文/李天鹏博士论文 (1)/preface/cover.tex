...
动态网络社团检测及其演化分析可以有效地挖掘复杂网络的结构和功能及其演化模式，能够广泛应用于城市规划、推荐系统、突发事件检测等领域。动态网络社团检测任务的核心是对动态网络演化模式的有效建模，现有方法往往启发式地设计模型对动态网络演化模式的刻画，不仅缺少实证支撑、难以扩展，还忽略了网络的高阶生成机理，无法有效支撑社团演化分析。针对上述问题，本文在动态随机块模型的基础上，从实证出发，挖掘动态网络演化机理，研究基于概率模型的动态网络社团检测与演化分析问题。其主要研究内容和创新性成果包括：
...
第二，针对动态随机块模型下游任务扩展不足，提出了动态网络层次演化异常检测方法，引入节点流行度参数以同时捕获节点在拓扑与演化层面的异质性，并提出动态网络层次演化异常检测指标以识别动态网络节点、社团、网络快照各层次的演化异常，以扩展模型对动态网络演化分析的维度，并在真实世界中进行了验证与演化异常挖掘。


第三，提出了基于深度生成模型的动态网络社团检测算法。针对随机块模型建模精度提升造成模型参数过多、模型参数后验推断复杂而造成模型对大规模数据处理能力差的核心问题，通过引入深度生成模型中的变分自编码器架构扩展了动态随机块模型的大规模数据应用能力。

本文从动态网络的高阶生成机理出发，以动态网络演化异质性实证挖掘为支撑，基于随机块模型设计了动态网络社团检测与演化分析方法，扩展了随机块模型的演化分析深度，并依托深度生成模型为动态随机块模型的大规模数据应用提供了方法范式，并利用大量实验与分析验证了所提出方法的有效性与实用性。
